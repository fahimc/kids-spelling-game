...
const SPELLING_WORDS = ['apple', 'banana', 'cherry', 'date', 'elderberry'];

function LearningPage() {
  const navigate = useNavigate();
  const { speak, isSpeaking } = useSpeechSynthesis();
  const [currentWordIndex, setCurrentWordIndex] = useState(0);
  const currentWord = SPELLING_WORDS[currentWordIndex];
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-3xl font-semibold mb-4">{currentWord}</h2>
        <button
      ...
      >
        ...
        <p className="text-gray-600">Interactive activities (tracing, missing letters) will go here in a later phase.</p>
      ...
      >
        {currentWordIndex < SPELLING_WORDS.length - 1 ? 'Next Word' : 'Finish Learning'}
      </motion.button>

       {currentWordIndex === SPELLING_WORDS.length - 1 && (
         <motion.button
